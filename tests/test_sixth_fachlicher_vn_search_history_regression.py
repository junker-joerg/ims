...
def _search_history_snapshot() -> dict:
    return {
        "policyholder_id": 21,
        ...
        "history": [
            {"period": 4, "sector_index": 0, "insured": True, "insurer_id": 12, "premium": 4.0},
            {"period": 4, "sector_index": 1, "insured": True, "insurer_id": 11, "premium": 5.0},
        ],
    }
...
def _vn_period_scenario() -> dict:
    return {
        ...
        "vn_insurance_rule_snapshots": [_search_history_snapshot()],
...
def test_sixth_fachlicher_vn_search_history_snapshot_regression() -> None:
    snapshots = load_vn_insurance_rule_snapshots_from_mapping([_search_history_snapshot()])
    ...
    assert application.rule_kind is VNInsuranceRuleKind.SEARCH_HISTORY
    ...
    assert application.result.selected_history_periods == [4, 4]
    assert application.result.used_fallback == [False, False]
    assert application.result.fallback_insurer_choice_draws is None


def test_sixth_fachlicher_vn_search_history_snapshot_feeds_runner_boundary() -> None:
