...
import ims.api.historical_500_period_vu_class_delivery as delivery_module
from ims.api.historical_500_period_vu_class_delivery import (
    CLASS_FILENAMES,
    EXPECTED_COMPARISON,
    build_historical_500_period_vu_class_delivery,
    main,
)
...
def test_vu_class_diagnostics_use_five_separate_100_period_runs(
    repeat_corpus,
) -> None:
    payload = build_historical_500_period_vu_class_delivery(
        REPO_ROOT,
        repeat_corpus=repeat_corpus,
    ).to_dict()

    assert payload["status"] == "ready"
    assert payload["mode"] == "historical_500_row_vu_class_repeat_diagnostics"
...
def test_vu_class_diagnostics_freeze_current_observations(repeat_corpus) -> None:
    payload = build_historical_500_period_vu_class_delivery(
        REPO_ROOT,
        repeat_corpus=repeat_corpus,
    ).to_dict()
    ...
    assert payload["historical_matched_row_count"] == 5
    assert payload["historical_mismatched_row_count"] == 1495
    assert payload["historical_compared_field_count"] == 21000
    assert payload["historical_exact_field_match_count"] == 3330
    assert payload["historical_tolerated_numeric_difference_count"] == 41
    assert payload["historical_blocking_numeric_difference_count"] == 17629
    ...
    for filename, expected in EXPECTED_COMPARISON.items():
        ...
        assert target["subject_type"] == "insurer"
...
def test_vu_class_diagnostics_complete_delivery_but_keep_release_closed(
    repeat_corpus,
) -> None:
    payload = build_historical_500_period_vu_class_delivery(
        REPO_ROOT,
    ...
    assert payload["cumulative_delivered_export_count"] == 15
    assert payload["cumulative_delivered_period_count"] == 6300
    assert payload["missing_export_count"] == 0
    assert payload["missing_period_count"] == 0
    assert payload["production_corpus_status"] == "blocked"
    assert payload["production_release_approved"] is False


def test_vu_class_diagnostics_reject_repeat_boundary_drift(repeat_corpus) -> None:
...
def test_vu_class_diagnostics_reject_shortened_result_rows(repeat_corpus) -> None:
    tables = [
        ExportTable(
            spec=table.spec,
            header=table.header,
...
def test_vu_class_diagnostics_reject_missing_target(repeat_corpus) -> None:
    tables = [
        table
        for table in repeat_corpus.export_tables
...
def test_vu_class_diagnostics_reject_reference_hash_drift(
    monkeypatch: pytest.MonkeyPatch,
    repeat_corpus,
) -> None:
    monkeypatch.setitem(delivery_module.REFERENCE_SHA256, "imsvuvk1.dat", "0" * 64)
...
def test_vu_class_diagnostics_cli_keeps_historical_claims_closed(
    monkeypatch: pytest.MonkeyPatch,
    repeat_corpus,
    capsys,
) -> None:
...
def test_vu_class_repeat_documentation_records_pr101_boundaries() -> None:
    document = (
        REPO_ROOT / "docs/migration/historical_500_period_vu_class_delivery.md"
    ...
    assert "3.330/21.000" in normalized
    assert "15/15 Tabellen" in normalized
    assert "PR102" in normalized
