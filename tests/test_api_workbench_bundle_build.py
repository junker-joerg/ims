...
def test_workbench_bundle_build_zip_smoke_covers_expected_bundle_boundaries(tmp_path):
    ...
    assert "scripts/workbench/test-release-gate.ps1" in names
    ...
    assert "docs/migration/windows_release_gate.md" in names
    assert "docs/migration/calculated_export_provenance_map.md" in names
...
def _build_repo_fixture(tmp_path: Path, *, include_frontend: bool = True) -> None:
    ...
    _touch(tmp_path / "docs" / "migration" / "calculated_export_provenance_map.md", "map")
    if include_frontend:
