...
def test_ims_core_resume_plan_names_next_reviewable_core_block() -> None:
    ...
    assert "vorgeschlagener naechster Schritt ist PR 82" in plan
    ...
    assert "mindestens `5`" in plan
    assert "Nach dem PR-81-Schocklauf bleiben `0` technische Pflicht-PRs" in plan
...
def test_production_readiness_plan_scopes_remaining_prs() -> None:
    ...
    assert "mindestens" in plan and "`5` weitere PRs" in plan
