...
def test_ims_core_resume_plan_names_next_reviewable_core_block() -> None:
    ...
    assert "PR 90 ist der naechste Umsetzungsschritt" in plan
    ...
    assert "Nach PR 89 bleiben `0` technische Pflicht-PRs" in plan
...
def test_production_readiness_plan_scopes_remaining_prs() -> None:
    ...
    assert "PR 88 und PR 89 haben das read-only" in plan.replace("\n", " ")
    assert "PR 90 wertet als naechstes Laufmetadaten" in plan.replace("\n", " ")
    assert "PR 90 bis PR 91" in plan
