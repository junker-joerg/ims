...
def test_ims_core_resume_plan_names_next_reviewable_core_block() -> None:
    ...
    assert "vorgeschlagener naechster Schritt ist PR 85" in plan
    assert "mindestens `2`" in plan
    ...
    assert "mindestens `2`" in plan
    assert "Nach dem PR-84-Regelgruppenlauf bleiben `0` technische Pflicht-PRs" in plan
...
def test_production_readiness_plan_scopes_remaining_prs() -> None:
    ...
    assert "mindestens" in plan and "`2` weitere PRs" in plan
    assert "PR 85 vergleicht als naechstes" in plan
