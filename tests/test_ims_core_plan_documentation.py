...
def test_ims_core_resume_plan_names_next_reviewable_core_block() -> None:
    ...
    assert "2 PRs bis zur demo-nahen read-only Carryover/Kern-Sicht" in plan
...
def test_plan_indexes_reference_ims_core_resume_plan() -> None:
    ...
    assert "2 PRs bis zur" in readme
    assert "3+ fachliche" in readme
    assert "5+ PRs bis zu einem breiteren fachlichen Anschluss" in readme
