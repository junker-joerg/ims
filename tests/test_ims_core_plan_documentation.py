...
def test_ims_core_resume_plan_names_next_reviewable_core_block() -> None:
    ...
    assert "PR 101 ist der naechste Umsetzungsschritt" in plan
    ...
    assert "Nach PR 100 bleiben `0` technische Pflicht-PRs" in plan
    assert "`2` geplante PRs" in plan
...
def test_production_readiness_plan_scopes_remaining_prs() -> None:
    ...
    assert "Nach PR 100 sind `2` PRs bis zum" in plan
    ...
    assert "Nach PR 100 sind `2` PRs" in plan
