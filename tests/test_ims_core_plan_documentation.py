...
def test_ims_core_resume_plan_names_next_reviewable_core_block() -> None:
    ...
    assert "PR 97 ist der naechste Umsetzungsschritt" in plan
    ...
    assert "Nach PR 96 bleiben `0` technische Pflicht-PRs" in plan
...
def test_production_readiness_plan_scopes_remaining_prs() -> None:
    ...
    assert "Nach PR 96 sind `5` PRs bis zum" in plan
    ...
    assert "PR 88 bis PR 96 haben das" in plan.replace("\n", " ")
    assert "zwei vollstaendigen 100er-Tabellen" in plan.replace("\n", " ")
    assert "Nach PR 96 sind `5` PRs" in plan
    assert "PR 97 bis PR 100" in plan
