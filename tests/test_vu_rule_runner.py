...
def _random_normal_parameters() -> dict[str, list[float]]:
    return {
        "premium_intercept_normal": [1.0, 2.0],
        "premium_factor_normal": [10.0, 20.0],
        "advertising_intercept_normal": [3.0, 4.0],
...
def test_vu_rule_runner_updates_targeted_insurers_and_returns_diagnostics() -> None:
    ...
    assert result.random_uniform_applications == []
    assert result.random_normal_applications == []
...
def test_vu_rule_runner_applies_random_rule_snapshots_after_foreign_info() -> None:
    ...
    assert result.rule_applications == []
    assert len(result.random_uniform_applications) == 1
    assert result.random_uniform_applications[0].insurer_id == 10
    assert len(result.random_normal_applications) == 1
    assert result.random_normal_applications[0].insurer_id == 11
    insurer_by_id = {insurer.entity_id: insurer for insurer in result.insurers}
    assert insurer_by_id[10].premiums_current_sector == pytest.approx([1.0, 4.0])
    assert insurer_by_id[10].advertising_current_sector == pytest.approx([9.0, 16.0])
    assert insurer_by_id[10].reserves_current == pytest.approx([52.5, 63.0])
...
def test_vu_rule_runner_counts_all_loaded_vu_rule_application_types_with_random_rules() -> None:
    ...
    scenario["insurers"].extend(
        [
            _additional_insurer(11),
            _additional_insurer(12),
            _additional_insurer(13),
...
def test_vu_rule_multi_period_runner_can_carry_current_insurer_state_forward() -> None:
    ...
    assert second.insurers[0].policyholders_prev_sector == [30.0, 80.0]


def test_vu_rule_multi_period_carryover_advances_net_switcher_previous_basis() -> None:
    ...
    assert result.period_results[1].insurers[0].policyholders_prev_sector == [30.0, 80.0]


def test_vu_rule_multi_period_carryover_uses_scalar_current_policyholders_for_net_switcher_basis() -> None:
    first = _scenario_for_period(2)
    first["insurers"][0].pop("policyholders_current_sector")
    first["insurers"][0]["policyholders_current"] = 42.0
    second = _scenario_for_period(3)
    second["insurers"][0].pop("policyholders_current_sector")
    second["vu_foreign_info_rule_snapshots"] = []
    second["vu_net_switcher_markup_rule_snapshots"] = [
        {
            "insurer_id": 10,
    ...
    application = result.period_results[1].net_switcher_markup_applications[0]
    assert application.result.net_switcher_values == pytest.approx([0.0, 0.0])
    assert result.period_results[1].insurers[0].policyholders_prev_sector == [42.0, 42.0]
    assert result.period_results[1].insurers[0].policyholders_current_sector == [42.0, 42.0]
