...
def test_plan_orders_provenance_before_full_window_execution() -> None:
    ...
    assert "Umsetzungsstand: PR 91" in plan
    assert "10 geplante PRs" in plan
    assert "Verbleibende grobe Bruttoabschaetzung fuer PR 92 bis PR 101" in plan
...
def test_plan_keeps_current_change_read_only_and_conservative() -> None:
    ...
    assert "1.700-4.200 LoC" in plan
...
def test_central_plans_and_index_reference_pr87_series() -> None:
    ...
    assert "Nach PR 91 sind `10` PRs" in production
    assert "PR 92 bis PR 100" in production
    assert "PR 101" in production
    assert "Nach PR 91" in core
    assert "`10` geplante PRs" in core
