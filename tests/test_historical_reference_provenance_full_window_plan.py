...
def test_plan_orders_provenance_before_full_window_execution() -> None:
    ...
    assert "Umsetzungsstand: PR 99" in plan
    assert "3 geplante PRs" in normalized
    assert "Verbleibende grobe Bruttoabschaetzung fuer PR 100 bis PR 102" in plan
...
def test_central_plans_and_index_reference_pr87_series() -> None:
    ...
    assert "Nach PR 99 sind `3` PRs" in production
    assert "PR 100 und PR 101" in production
    assert "PR 102" in production
    assert "Nach PR 99" in core
    assert "`3` geplante PRs" in core
