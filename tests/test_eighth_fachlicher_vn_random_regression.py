...
def _vn_period_scenario() -> dict:
    return {
        "context": {"period": 5, "max_periods": 6, "run_index": 0, "rng_seed": 12345},
        ...
        "vn_insurance_rule_snapshots": [_random_snapshot()],
...
def test_eighth_fachlicher_vn_random_snapshot_regression() -> None:
    snapshots = load_vn_insurance_rule_snapshots_from_mapping([_random_snapshot()])
    ...
    assert application.rule_kind is VNInsuranceRuleKind.RANDOM
    ...
    assert application.result.status_draws == [0.5, 0.1]
    assert application.result.insurer_choice_draws == [0.75, 0.0]


def test_eighth_fachlicher_vn_random_snapshot_feeds_runner_boundary() -> None:
