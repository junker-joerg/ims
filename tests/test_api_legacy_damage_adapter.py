...
def test_legacy_damage_adapter_contract_is_read_only(
    ...
    def reject_execution(*args: object, **kwargs: object) -> None:
        raise AssertionError("Der Adaptervertrag darf keinen Runner starten")

    monkeypatch.setattr(runner, "run_loaded_explicit_period", reject_execution)
    client = TestClient(create_app(frontend_dist=tmp_path))
    endpoint = "/api/model/legacy-damage-adapter-contract"

    response = client.get(endpoint)

    assert response.status_code == 200
    assert response.json() == legacy_damage_adapter_contract_payload()
