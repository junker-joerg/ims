...
def test_controlled_loop_processes_all_three_events() -> None:
    scenario_path = Path(__file__).parent / "fixtures" / "minimal_scenario.json"

    result = run_controlled_bav_event_loop(scenario_path, num_events=3, max_events=3)
...
def test_controlled_loop_stops_at_max_events_limit() -> None:
    scenario_path = Path(__file__).parent / "fixtures" / "minimal_scenario.json"

    result = run_controlled_bav_event_loop(scenario_path, num_events=5, max_events=3)
...
def test_controlled_loop_dispatch_order_follows_logtime_order() -> None:
    scenario_path = Path(__file__).parent / "fixtures" / "minimal_scenario.json"

    result = run_controlled_bav_event_loop(scenario_path, num_events=4, max_events=4)

    dispatched_logtimes = [item.event.logtime for item in result.dispatched_results]
    assert dispatched_logtimes == [0, 1, 2, 3]


def test_controlled_loop_seeded_rng_sequence_is_deterministic() -> None:
    scenario_path = Path(__file__).parent / "fixtures" / "minimal_scenario.json"
...
def test_controlled_loop_rejects_invalid_num_events() -> None:
    scenario_path = Path(__file__).parent / "fixtures" / "minimal_scenario.json"

    with pytest.raises(ValueError, match="num_events must be greater than 0"):
        run_controlled_bav_event_loop(scenario_path, num_events=0, max_events=1)


def test_controlled_loop_rejects_invalid_max_events() -> None:
    scenario_path = Path(__file__).parent / "fixtures" / "minimal_scenario.json"

    with pytest.raises(ValueError, match="max_events must be greater than 0"):
        run_controlled_bav_event_loop(scenario_path, num_events=1, max_events=0)


def test_controlled_loop_raises_when_sampling_without_rng() -> None:
    scenario_path = Path(__file__).parent / "fixtures" / "minimal_scenario.json"
