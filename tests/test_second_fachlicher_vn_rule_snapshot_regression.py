...
def test_second_fachlicher_vn_best_info_snapshot_regression() -> None:
    snapshots = load_vn_insurance_rule_snapshots_from_mapping([_best_info_snapshot()])
    ...
    assert application.rule_kind is VNInsuranceRuleKind.BEST_INFO
    ...
    assert application.result.considered_insurer_ids == [[11, 12], [11, 12]]
    assert application.result.information_cost == 4.0


def test_second_fachlicher_vn_best_info_snapshot_feeds_runner_boundary() -> None:
