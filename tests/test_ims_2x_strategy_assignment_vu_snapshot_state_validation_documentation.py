...
def test_pr120_documentation_records_sources_state_and_boundaries() -> None:
    ...
    for source in (
        ...
        "Vn[gperiod-2]",
        "akvn",
        "aspiration_sector_1/2",
    ):
        assert source in migration
    assert "draw_values_cross_checked_against_draw_plan = false" in migration
    assert "authentifiziert keinen historischen Lauf" in plan
    ...
    assert "PR121" in plan
    assert "PR122" in plan


def test_pr120_documentation_is_linked_from_all_strategy_indexes() -> None:
