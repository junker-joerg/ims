...
def test_core_placeholders_import() -> None:
    ...
    from ims.api.run_control_queue_overview import (
        ...
        run_control_queue_detail_payload,
    ...
    from ims.api.workbench_portable_readiness import (
        ...
        main as workbench_portable_readiness_main,
    )
    ...
    assert metadata_consistency_payload(list_scenario_metadata(), list_run_metadata(), metadata_capabilities())["status"] == "ok"
    ...
    assert ControlledExecutionAdapterResult is not None
    ...
    assert RunControlAdapterStartContract is not None
    ...
    assert run_control_queue_detail_payload is not None
    ...
    assert WorkbenchArtifactManifestFile is not None
    ...
    assert build_workbench_build_snapshot is not None
    assert workbench_build_snapshot_main is not None
    assert WorkbenchBundlePlanIssue is not None
    assert WorkbenchBundlePlanResult is not None
    assert build_workbench_bundle_plan is not None
    assert workbench_bundle_plan_main is not None
    ...
    assert build_workbench_portable_readiness is not None
    assert workbench_portable_readiness_main is not None
    ...
    assert ScheduledExplicitMultiPeriodResult is not None
