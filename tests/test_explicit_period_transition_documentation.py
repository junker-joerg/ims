...
def test_explicit_period_transition_diagnostics_is_documented() -> None:
    ...
    assert "python -m ims.engine.explicit_transition_carryover_probe --apply-vn" in doc
    assert 'mode = "explicit_period_transition_diagnostics"' in doc
    assert 'mode = "explicit_transition_carryover_probe"' in doc
    assert "transition_count" in doc
    assert "from_global_period" in doc
    assert "to_global_period" in doc
    assert "in_memory_carryover_performed" in doc
    assert 'previous_result_source = "explicit_fixture_snapshot"' in doc
    ...
    assert "diagnostic_candidate_ids_match = true" in doc
    assert "Versicherer `11` und den VN `21`" in doc
    ...
    assert "kein historisches Vorperioden-Ergebnis erfinden" in doc
...
def test_explicit_period_transition_diagnostics_document_is_listed() -> None:
    ...
    assert "rein lesende Uebergangsdiagnose und enger In-Memory-Carryover-Probe" in readme
