...
def test_active_roadmap_numbers_pr142_through_pr192_without_gaps() -> None:
    ...
    assert "PR161 Tod und Kapital umgesetzt, PR162 Neugeschaeft naechster Schritt" in document
    ...
    assert "kontrollierte Managementseminar-Reife | PR190 | 49 | 29" in document
    assert "Windows Ready-to-run ohne Zielrechner-Python | PR192 | 51 | 31" in document
...
def test_later_windows_ready_to_run_plan_keeps_pr162_next() -> None:
    roadmap = ROADMAP.read_text(encoding="utf-8")
    plan = WINDOWS_PLAN.read_text(encoding="utf-8")

    assert "PR161 Tod und Kapital umgesetzt, PR162 Neugeschaeft naechster Schritt" in roadmap
    assert "PR162 bleibt der" in plan
    for phrase in (
