...
def test_active_roadmap_numbers_pr142_through_pr192_without_gaps() -> None:
    ...
    assert "PR165 Lebensanschluss umgesetzt, PR166 Ergebnisablage naechster Schritt" in document
    ...
    assert "kontrollierte Managementseminar-Reife | PR190 | 49 | 25" in document
    assert "Windows Ready-to-run ohne Zielrechner-Python | PR192 | 51 | 27" in document
...
def test_later_windows_ready_to_run_plan_keeps_pr166_next() -> None:
    ...
    assert "PR166 bleibt der" in plan
    for phrase in (
