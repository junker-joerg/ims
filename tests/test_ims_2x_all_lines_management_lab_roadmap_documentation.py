...
def test_active_roadmap_numbers_pr142_through_pr192_without_gaps() -> None:
    ...
    assert "PR160 umgesetzt, PR161 Tod und Kapital naechster Schritt" in document
    ...
    assert "kontrollierte Managementseminar-Reife | PR190 | 49 | 30" in document
    assert "Windows Ready-to-run ohne Zielrechner-Python | PR192 | 51 | 32" in document
...
def test_later_windows_ready_to_run_plan_keeps_pr161_next() -> None:
    roadmap = ROADMAP.read_text(encoding="utf-8")
    plan = WINDOWS_PLAN.read_text(encoding="utf-8")

    assert "PR160 umgesetzt, PR161 Tod und Kapital naechster Schritt" in roadmap
    assert "PR161 bleibt der" in plan
    for phrase in (
