...
def test_active_roadmap_numbers_pr142_through_pr192_without_gaps() -> None:
    ...
    assert "PR169a Kranken-Bestands- und Quellenvertrag umgesetzt, PR169b Periodenkette naechster Schritt" in document
    ...
    assert "bedienbare Kranken-Simulation bis 100 Perioden | PR169d | 32 | 3" in document
    assert "kontrollierte Managementseminar-Reife | PR190 | 53 | 24" in document
    assert "Windows Ready-to-run ohne Zielrechner-Python | PR192 | 55 | 26" in document
...
def test_later_windows_ready_to_run_plan_keeps_pr169b_next() -> None:
    roadmap = ROADMAP.read_text(encoding="utf-8")
    plan = WINDOWS_PLAN.read_text(encoding="utf-8")

    assert "PR169a Kranken-Bestands- und Quellenvertrag umgesetzt, PR169b Periodenkette naechster Schritt" in roadmap
    assert "PR169b bleibt der" in plan
    for phrase in (
