...
def test_active_roadmap_numbers_pr142_through_pr192_without_gaps() -> None:
    ...
    assert "PR164 Annahmen umgesetzt, PR165 Lebensanschluss naechster Schritt" in document
    ...
    assert "kontrollierte Managementseminar-Reife | PR190 | 49 | 26" in document
    assert "Windows Ready-to-run ohne Zielrechner-Python | PR192 | 51 | 28" in document
...
def test_later_windows_ready_to_run_plan_keeps_pr165_next() -> None:
    roadmap = ROADMAP.read_text(encoding="utf-8")
    plan = WINDOWS_PLAN.read_text(encoding="utf-8")

    assert "PR164 Annahmen umgesetzt, PR165 Lebensanschluss naechster Schritt" in roadmap
    assert "PR165 bleibt der" in plan
    for phrase in (
