...
def test_horizon_limits_are_explicit_and_not_released(
    periods: int,
    seconds: int,
) -> None:
...
def test_horizon_contract_fails_closed_and_preserves_five_period_path() -> None:
    ...
    assert [definition.period_count for definition in STRATEGY_EXECUTION_HORIZON_DEFINITIONS] == [
        10, 25, 50, 100
    ]
    assert payload["released_period_counts"] == [2, 5]
    assert payload["contract_only_period_counts"] == [10, 25, 50, 100]
    ...
    assert payload["budget_policy"]["python_thread_cancellation_is_not_sufficient"] is True
    assert payload["preflight_policy"]["incomplete_preflight_blocks_first_runner"] is True
...
def test_contract_payload_is_fresh_and_cannot_mutate_future_calls() -> None:
    ...
    assert second["contract_only_period_counts"] == [10, 25, 50, 100]
