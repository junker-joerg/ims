...
def test_pr140_plan_and_mapping_define_controlled_persistent_boundary() -> None:
    ...
    assert "genau zwei Perioden" in normalized
    assert "BEGIN IMMEDIATE" in normalized
    assert "Idempotenz" in normalized
    assert "kein Ergebnis aus Periode 1" in normalized
