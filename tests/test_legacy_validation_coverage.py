...
def test_legacy_validation_coverage_matrix_summarizes_current_bundle() -> None:
    ...
    assert payload["backlog"][3]["family"] == "insurer_class"
    assert payload["backlog"][3]["available_files"] == [
        "IMSVUVK1.DAT",
        "IMSVUVK2.DAT",
        "IMSVUVK3.DAT",
    ]
    assert payload["backlog"][3]["covered_files"] == [
        "IMSVUVK1.DAT",
        "IMSVUVK2.DAT",
        "IMSVUVK3.DAT",
    ]
    assert payload["backlog"][3]["missing_files"] == []
...
def test_legacy_validation_coverage_matrix_reports_available_uncovered_reference(
    ...
    assert payload["reference_count"] == 18
    assert payload["covered_rows"] == 6200
    assert payload["gaps"] == [
        {
