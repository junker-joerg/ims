...
def _write_clean_diagnostic_summary_directory(tmp_path: Path) -> Path:
    run_legacy_validation_batch_from_fixture(
    ...
    return tmp_path / "summary_artifacts"
...
def test_legacy_validation_fixture_runs_multiple_file_families(tmp_path: Path) -> None:
    ...
    assert result.report.total_files == 19
    assert result.report.total_rows == 6300
    assert result.report.matched_rows == 6300
    ...
    assert payload["files"][5]["selector_kind"] == "entity"
    assert payload["files"][5]["selector_value"] == 14
    assert payload["files"][5]["end_period"] == 100
    assert payload["files"][6]["filename"] == "imsvnsk1.dat"
    assert payload["files"][6]["subject_type"] == "policyholder"
    assert payload["files"][6]["level"] == "IV"
    assert payload["files"][6]["selector_kind"] == "all"
    assert payload["files"][6]["selector_value"] == "SK1"
    assert payload["files"][7]["filename"] == "imsvnr01.dat"
    assert payload["files"][7]["start_period"] == 1
    assert payload["files"][7]["end_period"] == 300
    assert payload["files"][7]["selector_value"] == 1
    assert payload["files"][8]["filename"] == "imsvnr02.dat"
    assert payload["files"][8]["end_period"] == 300
    assert payload["files"][8]["selector_value"] == 2
    assert payload["files"][9]["filename"] == "imsvnr03.dat"
    assert payload["files"][9]["end_period"] == 500
    assert payload["files"][9]["selector_value"] == 3
    assert payload["files"][10]["filename"] == "imsvnr04.dat"
    assert payload["files"][10]["end_period"] == 500
    assert payload["files"][10]["selector_value"] == 4
    assert payload["files"][11]["filename"] == "imsvnr05.dat"
    assert payload["files"][11]["end_period"] == 500
    assert payload["files"][11]["selector_value"] == 5
    ...
    assert payload["files"][16]["filename"] == "imsvuvk1.dat"
    assert payload["files"][16]["subject_type"] == "insurer"
    assert payload["files"][16]["level"] == "III"
    assert payload["files"][16]["selector_kind"] == "rule_class"
    assert payload["files"][16]["end_period"] == 500
    assert payload["files"][16]["selector_value"] == 1
    assert payload["files"][17]["filename"] == "imsvuvk2.dat"
    assert payload["files"][17]["subject_type"] == "insurer"
    assert payload["files"][17]["level"] == "III"
    assert payload["files"][17]["selector_kind"] == "rule_class"
    assert payload["files"][17]["end_period"] == 500
    assert payload["files"][17]["selector_value"] == 2
    assert payload["files"][18]["filename"] == "imsvuvk3.dat"
    assert payload["files"][18]["subject_type"] == "insurer"
    assert payload["files"][18]["level"] == "III"
    assert payload["files"][18]["selector_kind"] == "rule_class"
    assert payload["files"][18]["end_period"] == 500
    assert payload["files"][18]["selector_value"] == 3
    ...
    assert rows[5]["level"] == "I"
    assert rows[5]["selector_kind"] == "entity"
    assert rows[5]["selector_value"] == "14"
    assert rows[6]["level"] == "IV"
    assert rows[6]["selector_kind"] == "all"
    assert rows[6]["selector_value"] == "SK1"
    assert rows[7]["filename"] == "imsvnr01.dat"
    assert rows[7]["selector_value"] == "1"
    assert rows[8]["filename"] == "imsvnr02.dat"
    assert rows[8]["selector_value"] == "2"
    assert rows[9]["filename"] == "imsvnr03.dat"
    assert rows[9]["selector_value"] == "3"
    assert rows[10]["filename"] == "imsvnr04.dat"
    assert rows[10]["selector_value"] == "4"
    assert rows[11]["filename"] == "imsvnr05.dat"
    ...
    assert rows[16]["filename"] == "imsvuvk1.dat"
    assert rows[16]["subject_type"] == "insurer"
    assert rows[16]["selector_kind"] == "rule_class"
    assert rows[16]["selector_value"] == "1"
    assert rows[17]["filename"] == "imsvuvk2.dat"
    assert rows[17]["subject_type"] == "insurer"
    assert rows[17]["selector_kind"] == "rule_class"
    assert rows[17]["selector_value"] == "2"
    assert rows[18]["filename"] == "imsvuvk3.dat"
    assert rows[18]["subject_type"] == "insurer"
    assert rows[18]["selector_kind"] == "rule_class"
    assert rows[18]["selector_value"] == "3"
    ...
    assert period_rows[0]["filenames"] == "imsvusk1.dat;imsvnr03.dat;imsvnr04.dat;imsvnr05.dat;imsvnr06.dat;imsvnvk1.dat;imsvnvk2.dat;imsvnvk3.dat;imsvuvk1.dat;imsvuvk2.dat;imsvuvk3.dat"
    assert period_rows[400]["global_period"] == "1"
    assert period_rows[400]["row_count"] == "15"
    ...
    assert manifest["total_files"] == 19
    assert manifest["total_rows"] == 6300
    ...
    assert loaded_manifest.total_rows == 6300
    ...
    assert report_payload["total_rows"] == 6300
    assert report_payload["total_files"] == 19
    ...
    assert report_summary.total_files == 19
    assert report_summary.total_rows == 6300
    assert report_summary.matched_rows == 6300
...
def test_legacy_validation_report_payload_summary_tracks_deviations(tmp_path: Path) -> None:
    ...
    report_data["matched_rows"] = 6299
    report_data["mismatched_rows"] = 1
    report_data["match_rate"] = 6299 / 6300
    report_data["files"][5]["matches"] = False
    report_data["files"][5]["matched_rows"] = 99
    report_data["files"][5]["mismatched_rows"] = 1
    report_data["files"][5]["match_rate"] = 0.99
    report_data["files"][5]["periods_with_differences"] = [2]
    report_data["files"][5]["fields_with_differences"] = ["Pr1"]
    ...
    manifest_data["matched_rows"] = 6299
    ...
    assert summary.matched_rows == 6299
    assert summary.mismatched_rows == 1
    assert summary.match_rate == 6299 / 6300
...
def test_legacy_validation_report_payload_summary_bundle_from_manifests(tmp_path: Path) -> None:
    ...
    assert bundle.total_files == 38
    assert bundle.total_rows == 12600
    assert bundle.matched_rows == 12600
...
def test_legacy_validation_report_payload_summary_bundle_tracks_mixed_results(tmp_path: Path) -> None:
    ...
    assert bundle.total_files == 38
    assert bundle.total_rows == 12600
    assert bundle.matched_rows == 12599
    assert bundle.mismatched_rows == 1
    assert bundle.match_rate == 12599 / 12600
...
def test_legacy_validation_report_payload_summary_bundle_writes_json_and_csv(tmp_path: Path) -> None:
    ...
    assert payload["total_files"] == 38
    assert payload["total_rows"] == 12600
    ...
    assert rows[0]["total_files"] == "19"
    assert rows[0]["total_rows"] == "6300"
...
def test_legacy_validation_report_summary_bundle_artifacts_roundtrip(tmp_path: Path) -> None:
    ...
    assert manifest.total_files == 38
    assert manifest.total_rows == 12600
    ...
    assert loaded_manifest.total_rows == 12600
...
def test_legacy_validation_report_summary_bundle_artifacts_from_directory(tmp_path: Path) -> None:
    ...
    assert manifest.total_files == 38
    assert manifest.total_rows == 12600
    assert manifest.matched_rows == 12600
    assert manifest.artifact_count == 3
...
def test_legacy_validation_batch_fixture_runs_items_and_writes_summary(tmp_path: Path) -> None:
    ...
    assert [item.result.report.total_rows for item in result.runs] == [6300, 6300]
    ...
    assert summary_manifest.total_files == 38
    assert summary_manifest.total_rows == 12600
    assert summary_manifest.matched_rows == 12600
    ...
    assert batch_manifest_payload["total_rows"] == 12600
...
def test_legacy_validation_acceptance_run_artifacts_from_summary_directory(
    ...
    assert result.run_manifest_path == tmp_path / "acceptance" / "legacy_validation_acceptance_run.json"
    ...
    run_manifest = load_legacy_validation_acceptance_run_manifest(
        result.run_manifest_path
    )
    assert isinstance(run_manifest, LegacyValidationAcceptanceRunManifest)
    assert run_manifest.run_name == "legacy_validation_acceptance_run"
    assert run_manifest.passed is True
    assert run_manifest.total_runs == 6
...
def test_legacy_validation_acceptance_run_artifacts_from_summary_directory_tracks_failure(
    ...
    run_manifest = load_legacy_validation_acceptance_run_manifest(
        acceptance.run_manifest_path
    )
    assert run_manifest.passed is False
    assert run_manifest.issue_count == 1


def test_legacy_validation_acceptance_run_manifest_rejects_verdict_mismatch(
    tmp_path: Path,
) -> None:
    acceptance = write_legacy_validation_acceptance_run_artifacts_from_summary_directory(
        _write_clean_diagnostic_summary_directory(tmp_path),
        tmp_path / "acceptance",
    )
    payload = json.loads(acceptance.run_manifest_path.read_text(encoding="utf-8"))
    payload["total_runs"] = 999
    acceptance.run_manifest_path.write_text(json.dumps(payload), encoding="utf-8")

    try:
        load_legacy_validation_acceptance_run_manifest(acceptance.run_manifest_path)
    except ValueError as exc:
        assert "verdict field total_runs" in str(exc)
    else:
        raise AssertionError("acceptance run manifest mismatch should fail")


def test_legacy_validation_acceptance_run_manifest_rejects_missing_artifacts(
    tmp_path: Path,
) -> None:
    acceptance = write_legacy_validation_acceptance_run_artifacts_from_summary_directory(
        _write_clean_diagnostic_summary_directory(tmp_path),
    ...
    acceptance.verdict_manifest.artifacts[-1].path.unlink()
    ...
    try:
        ...
        raise AssertionError("missing acceptance run artifact should fail")
