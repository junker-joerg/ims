...
def test_legacy_validation_fixture_runs_multiple_file_families(tmp_path: Path) -> None:
    ...
    assert result.report.total_rows == 400
    assert result.report.matched_rows == 400
    ...
    assert payload["files"][1]["end_period"] == 100
    ...
    assert payload["period_summaries"][100]["global_period"] == 1
    assert payload["period_summaries"][100]["filenames"] == [
        "imsvu014.dat",
        "imsvnsk1.dat",
        "imsvnr05.dat",
    ]
    ...
    assert group_rows[0]["row_count"] == "100"
    ...
    assert period_rows[100]["global_period"] == "1"
    assert period_rows[100]["row_count"] == "3"
    ...
    assert manifest["total_rows"] == 400
    ...
    assert loaded_manifest.total_rows == 400
    ...
    assert report_payload["total_rows"] == 400
    ...
    assert report_summary.total_rows == 400
    assert report_summary.matched_rows == 400
...
def test_legacy_validation_report_payload_summary_bundle_from_manifests(tmp_path: Path) -> None:
    ...
    assert bundle.total_rows == 800
    assert bundle.matched_rows == 800
...
def test_legacy_validation_report_payload_summary_bundle_tracks_mixed_results(tmp_path: Path) -> None:
    ...
    report_data = json.loads(report_path.read_text(encoding="utf-8"))
    report_data["matches"] = False
    report_data["matched_rows"] = 399
    report_data["mismatched_rows"] = 1
    report_data["match_rate"] = 0.9975
    report_data["files"][1]["matches"] = False
    report_data["files"][1]["matched_rows"] = 99
    report_data["files"][1]["mismatched_rows"] = 1
    report_data["deviation_index"] = [
        {
            "filename": "imsvu014.dat",
    ...
    manifest_data = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest_data["matches"] = False
    manifest_data["matched_rows"] = 399
    manifest_data["mismatched_rows"] = 1
    manifest_path.write_text(json.dumps(manifest_data), encoding="utf-8")
    ...
    assert bundle.total_files == 8
    assert bundle.total_rows == 800
    assert bundle.matched_rows == 799
    assert bundle.mismatched_rows == 1
    assert bundle.match_rate == 799 / 800
...
def test_legacy_validation_report_summary_bundle_artifacts_from_directory(tmp_path: Path) -> None:
    ...
    assert manifest.total_files == 8
    assert manifest.total_rows == 800
    assert manifest.matched_rows == 800
    assert manifest.artifact_count == 3
...
def test_legacy_validation_batch_fixture_runs_items_and_writes_summary(tmp_path: Path) -> None:
    ...
    assert [item.result.report.total_rows for item in result.runs] == [400, 400]
    ...
    assert summary_manifest.total_rows == 800
    assert summary_manifest.matched_rows == 800
    ...
    assert batch_manifest_payload["total_rows"] == 800
