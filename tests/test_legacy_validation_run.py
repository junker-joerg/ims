...
def test_legacy_validation_fixture_runs_multiple_file_families(tmp_path: Path) -> None:
    ...
    assert result.report.total_files == 8
    assert result.report.total_rows == 800
    assert result.report.matched_rows == 800
    ...
    assert manifest["total_rows"] == 800
    ...
    assert loaded_manifest.total_rows == 800
    ...
    assert report_payload["total_rows"] == 800
    assert report_payload["total_files"] == 8
    ...
    assert report_summary.total_files == 8
    assert report_summary.total_rows == 800
    assert report_summary.matched_rows == 800
...
def test_legacy_validation_report_payload_summary_tracks_deviations(tmp_path: Path) -> None:
    ...
    report_data["matched_rows"] = 799
    report_data["mismatched_rows"] = 1
    report_data["match_rate"] = 799 / 800
    ...
    manifest_data["matched_rows"] = 799
    ...
    assert summary.matched_rows == 799
    assert summary.mismatched_rows == 1
    assert summary.match_rate == 799 / 800
...
def test_legacy_validation_report_payload_summary_bundle_from_manifests(tmp_path: Path) -> None:
    ...
    assert bundle.total_files == 16
    assert bundle.total_rows == 1600
    assert bundle.matched_rows == 1600
...
def test_legacy_validation_report_payload_summary_bundle_tracks_mixed_results(tmp_path: Path) -> None:
    ...
    assert bundle.total_files == 16
    assert bundle.total_rows == 1600
    assert bundle.matched_rows == 1599
    assert bundle.mismatched_rows == 1
    assert bundle.match_rate == 1599 / 1600
...
def test_legacy_validation_report_payload_summary_bundle_writes_json_and_csv(tmp_path: Path) -> None:
    ...
    assert payload["total_rows"] == 1600
    ...
    assert rows[0]["total_files"] == "8"
    assert rows[0]["total_rows"] == "800"
...
def test_legacy_validation_report_summary_bundle_artifacts_roundtrip(tmp_path: Path) -> None:
    ...
    assert manifest.total_rows == 1600
    ...
    assert loaded_manifest.total_rows == 1600
...
def test_legacy_validation_report_summary_bundle_artifacts_from_directory(tmp_path: Path) -> None:
    ...
    assert manifest.total_files == 16
    assert manifest.total_rows == 1600
    assert manifest.matched_rows == 1600
    assert manifest.artifact_count == 3
...
def test_legacy_validation_batch_fixture_runs_items_and_writes_summary(tmp_path: Path) -> None:
    ...
    assert [item.result.report.total_rows for item in result.runs] == [800, 800]
    ...
    assert summary_manifest.total_files == 16
    assert summary_manifest.total_rows == 1600
    assert summary_manifest.matched_rows == 1600
    ...
    assert batch_manifest_payload["total_rows"] == 1600
