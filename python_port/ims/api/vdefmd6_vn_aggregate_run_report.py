...
CONTRACT_VERSION = "pr85-v1"
CONTRACT_BASE_SEED = 20260001
DEFAULT_CONTRACT_PATH = Path(
    "tests/fixtures/vdefmd6_vn_aggregate_run_contract.json"
)
DEFAULT_REFERENCE_PATHS = {
    "imsvnsk1.dat": Path("tests/references/legacy_agrsich/IMSVNSK1.DAT"),
    "imsvnvk1.dat": Path("tests/references/legacy_agrsich/IMSVNVK1.DAT"),
    "imsvnvk2.dat": Path("tests/references/legacy_agrsich/IMSVNVK2.DAT"),
    "imsvnvk3.dat": Path("tests/references/legacy_agrsich/IMSVNVK3.DAT"),
}
_BLOCKER_CODES = (
...
def build_vdefmd6_vn_aggregate_run_report(
    repo_root: Path | str,
    *,
    contract_path: Path | str | None = None,
    reference_paths: dict[str, Path | str] | None = None,
) -> Vdefmd6VNAggregateRunReport:
    ...
    return Vdefmd6VNAggregateRunReport(
        repo_root=str(root),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m ims.api.vdefmd6_vn_aggregate_run_report",
        description="Prueft die vier kontrollierten PR-85-VN-Aggregate.",
    )
    parser.add_argument("--repo-root", type=Path, default=Path.cwd())
    parser.add_argument("--contract", type=Path)
    args = parser.parse_args(argv)
    report = build_vdefmd6_vn_aggregate_run_report(
        args.repo_root,
...
def _build_target_summaries(
    ...
    if set(tables) != set(VDEFMD6_VN_AGGREGATE_FILENAMES):
        _issue(
            issues,
        ...
        return ()
    if set(reference_paths) != set(VDEFMD6_VN_AGGREGATE_FILENAMES):
        _issue(
            issues,
...
def _validate_contract(
    ...
    expected = {
        ...
        "model_id": "Vdefmd6",
        "scope": "controlled_vn_aggregates_periods_1_100",
