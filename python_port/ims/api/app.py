...
from ims.api.strategy_execution_period_chain_five_period_build import (
    ...
    strategy_execution_five_period_chain_build_error_payload,
)
...
from ims.api.strategy_execution_period_chain_effect_probe_start import (
    ...
    strategy_execution_period_chain_effect_probe_start_error_payload,
)
...
def _strategy_assignment_snapshot_context_invalid_json_payload() -> dict[str, object]:
    return {
        "schema_version": STRATEGY_ASSIGNMENT_SNAPSHOT_CONTEXT_VALIDATION_VERSION,
        ...
        "all_context_values_supplied": False,
        "issue_count": 1,
        "issues": [
            {
                "path": "$",
                "code": "invalid_json",
                "message": "Snapshot-Kontextanfrage ist kein gueltiges JSON",
            }
        ],
        "defaults_applied": False,
...
def _strategy_assignment_vu_snapshot_state_validation_invalid_json_payload() -> dict[
    ...
    return {
        ...
        "mode": "strategy_assignment_vu_snapshot_state_validation",
        "status": "error",
        "valid": False,
        "input_valid": False,
        "state_shape_valid": False,
        "submitted_state_schema_version": None,
        "draft_id": None,
        "period": None,
        "expected_state_entry_count": 0,
        "validated_state_entry_count": 0,
        "expected_provenance_check_count": 0,
        "matched_provenance_check_count": 0,
        "expected_threshold_check_count": 0,
        "matched_threshold_check_count": 0,
        "issue_count": 1,
        "issues": [
            {
                ...
                "path": "$",
                "code": "invalid_json",
                "message": "VU-Zustandspruefung ist kein gueltiges JSON",
            }
        ],
        "threshold_values_cross_checked_against_actor_state": False,
        "draw_values_cross_checked_against_draw_plan": False,
        "state_values_inspected": False,
...
def _strategy_execution_period_chain_overview_error_payload(
    code: str,
    message: str,
) -> dict[str, object]:
    return {
        ...
        "period_chains": [],
        "all_period_chain_digests_verified": False,
        "issue_count": 1,
        "issues": [{"code": code, "message": message}],
        "writes_performed": False,
        "execution_performed": False,
...
def create_app(
    ...
    async def solvency_risk_modules_response(request: Request) -> JSONResponse:
        try:
            payload = await request.json()
        except ValueError:
            return JSONResponse(
    ...
    async def strategy_assignment_snapshot_context_validation_response(
        request: Request,
    ) -> JSONResponse:
        try:
            payload = await request.json()
            ...
            return JSONResponse(
                _strategy_assignment_snapshot_context_invalid_json_payload(),
                status_code=400,
            )
        return JSONResponse(
    ...
    async def strategy_assignment_vu_snapshot_state_validation_response(
        request: Request,
    ) -> JSONResponse:
        try:
            payload = await request.json()
            ...
            return JSONResponse(
                _strategy_assignment_vu_snapshot_state_validation_invalid_json_payload(),
                status_code=400,
            )
        return JSONResponse(
    ...
    async def strategy_execution_period_chain_resolution_response(
        request: Request,
    ) -> JSONResponse:
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            ...
            return JSONResponse(
                strategy_execution_period_chain_resolution_error_payload(
                    ...
                    "Periodenketten-Aufloesung erfordert eine explizite SQLite-Metadatenquelle",
                ),
                status_code=400,
            )
        try:
        ...
        if report.resolution_ready or not report.input_validated:
            status_code = 200
    ...
    async def strategy_execution_five_period_chain_build_response(
        request: Request,
    ) -> JSONResponse:
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            "path"
            ...
            return JSONResponse(
                strategy_execution_five_period_chain_build_error_payload(
                    "explicit_sqlite_store_required",
                    "Fuenf-Perioden-Bau erfordert eine explizite "
        ...
        try:
            ...
            return JSONResponse(
                strategy_execution_five_period_chain_build_error_payload(
                    "invalid_json",
                    "Fuenf-Perioden-Eingang ist kein gueltiges JSON",
                ),
                status_code=400,
            )
    ...
    def strategy_execution_period_chain_overview_response() -> JSONResponse:
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            "path"
        ):
            return JSONResponse(
        ...
        try:
            result = list_strategy_execution_period_chains(
                db_path=Path(str(metadata_source["path"])),
            )
        except StrategyExecutionPeriodChainStoreError as exc:
            return JSONResponse(
                _strategy_execution_period_chain_overview_error_payload(
                    exc.code,
                    str(exc),
                ),
                status_code=409,
    ...
    async def strategy_execution_period_chain_effect_probe_response(
        request: Request,
    ) -> JSONResponse:
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            "path"
            ...
            return JSONResponse(
                strategy_execution_period_chain_effect_probe_error_payload(
                    "explicit_sqlite_store_required",
                    "Zwei-Perioden-Wirkungsprobe erfordert eine explizite "
                    "SQLite-Metadatenquelle",
                ),
                status_code=400,
            )
        try:
            ...
            return JSONResponse(
                strategy_execution_period_chain_effect_probe_error_payload(
                    "invalid_json",
                    "Zwei-Perioden-Wirkungsprobe ist kein gueltiges JSON",
                ),
                status_code=400,
            )
        try:
            parsed = parse_strategy_execution_period_chain_effect_probe_request(
                payload
            )
        except StrategyExecutionPeriodChainEffectProbeError as exc:
            return JSONResponse(
                strategy_execution_period_chain_effect_probe_error_payload(
                    exc.code,
                    str(exc),
        ...
        if result.execution_performed:
            ...
            issue["code"]
            in {"period_chain_not_found", "period_chain_store_missing"}
            for issue in result.release_check.issues
        ):
            status_code = 404
        else:
            status_code = 409
    ...
    async def strategy_execution_five_period_effect_probe_response(
        ...
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            ...
            return JSONResponse(
                strategy_execution_five_period_effect_probe_error_payload(
                    "explicit_sqlite_store_required",
                    "Fuenf-Perioden-Wirkungsprobe erfordert eine explizite "
                    "SQLite-Metadatenquelle",
                ),
                status_code=400,
            )
        try:
            ...
            return JSONResponse(
                strategy_execution_five_period_effect_probe_error_payload(
                    "invalid_json",
                    "Fuenf-Perioden-Wirkungsprobe ist kein gueltiges JSON",
                ),
                status_code=400,
            )
        try:
            parsed = parse_strategy_execution_five_period_effect_probe_request(
                payload
            )
    ...
    async def strategy_execution_period_chain_effect_probe_start_response(
        request: Request,
    ) -> JSONResponse:
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            "path"
            ...
            return JSONResponse(
                strategy_execution_period_chain_effect_probe_start_error_payload(
                    "explicit_sqlite_store_required",
                    "Zwei-Perioden-Start erfordert eine explizite "
                    "SQLite-Metadatenquelle",
                ),
                status_code=400,
            )
        try:
            ...
            return JSONResponse(
                strategy_execution_period_chain_effect_probe_start_error_payload(
                    "invalid_json",
                    "Zwei-Perioden-Start ist kein gueltiges JSON",
                ),
                status_code=400,
            )
        try:
        ...
        try:
            result = start_strategy_execution_period_chain_effect_probe(
                parsed,
                db_path=Path(str(metadata_source["path"])),
                runner=period_chain_effect_probe_runner,
            )
    ...
    def strategy_execution_period_chain_effect_probe_result_response(
        ...
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            ...
            return JSONResponse(
                strategy_execution_period_chain_effect_probe_start_error_payload(
                    "explicit_sqlite_store_required",
                    "Zwei-Perioden-Ergebnisanzeige erfordert eine explizite "
                    "SQLite-Metadatenquelle",
                    chain_id=chain_id,
                ),
                status_code=400,
    ...
    def strategy_execution_period_chain_effect_probe_history_response(
        chain_id: str,
    ) -> JSONResponse:
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            "path"
            ...
            return JSONResponse(
                strategy_execution_period_chain_effect_probe_start_error_payload(
                    "explicit_sqlite_store_required",
                    "Zwei-Perioden-Versuchsverlauf erfordert eine explizite "
                    "SQLite-Metadatenquelle",
                    chain_id=chain_id,
                ),
                status_code=400,
        ...
        try:
            result = get_strategy_execution_period_chain_effect_probe_history(
                chain_id,
                db_path=Path(str(metadata_source["path"])),
            )
        except StrategyExecutionPeriodChainEffectProbeStartError as exc:
    ...
    async def strategy_execution_candidate_effect_probe_response(
        ...
        if metadata_source.get("storage_kind") != "sqlite" or not metadata_source.get(
            ...
            return JSONResponse(
                strategy_execution_candidate_effect_probe_error_payload(
                    "explicit_sqlite_store_required",
                    "Einperioden-Wirkungsprobe erfordert eine explizite SQLite-Metadatenquelle",
                ),
                status_code=400,
            )
        try:
            ...
            return JSONResponse(
                strategy_execution_candidate_effect_probe_error_payload(
                    "invalid_json",
                    "Einperioden-Wirkungsprobe ist kein gueltiges JSON",
                ),
                status_code=400,
            )
        try:
        ...
        if result.execution_performed:
            status_code = 200
        elif any(issue["code"] == "candidate_not_found" for issue in result.release_check.issues):
            status_code = 404
        else:
            status_code = 409
        return JSONResponse(result.to_dict(), status_code=status_code)
    ...
    if FastAPI is not None:
        ...
        @app.get("/api/model/health-sector-contract")
        ...
        @app.get("/api/model/life-sector-contract/v3")
        def model_life_sector_contract_v3() -> dict[str, object]:
            return life_sector_v3_contract_payload()
        ...
        @app.get("/api/accounting/model-balance-contract")
        ...
        @app.get("/api/accounting/solvency-risk-modules-contract")
        def accounting_solvency_risk_modules_contract() -> dict[str, object]:
            return solvency_risk_modules_contract_payload()

        @app.post("/api/accounting/solvency-risk-modules", response_model=None)
        ...
        @app.get("/api/strategies/assignment-snapshot-context-contract")
        def strategies_assignment_snapshot_context_contract() -> dict[str, object]:
            return strategy_assignment_snapshot_context_contract_payload()

        @app.get("/api/strategies/assignment-snapshot-materialization-contract")
        ...
        @app.get("/api/strategies/execution-period-chain-resolution-contract")
        def strategies_execution_period_chain_resolution_contract() -> dict[
            str, object
        ]:
            return strategy_execution_period_chain_resolution_contract_payload()

        @app.post(
            "/api/strategies/execution-period-chain-resolution",
            response_model=None,
        )
        async def strategies_execution_period_chain_resolution(
            request: Request,
        ) -> JSONResponse:
        ...
        def strategies_execution_five_period_chain_build_contract() -> dict[
            str, object
        ]:
            return strategy_execution_five_period_chain_build_contract_payload()

        @app.post(
            "/api/strategies/execution-period-chain-five-period-build",
            response_model=None,
        )
        async def strategies_execution_five_period_chain_build(
            request: Request,
        ) -> JSONResponse:
            return await strategy_execution_five_period_chain_build_response(request)
        ...
        def strategies_execution_period_chain_overview() -> JSONResponse:
            return strategy_execution_period_chain_overview_response()
        ...
        @app.get("/api/strategies/assignment-vu-snapshot-state-contract")
        def strategies_assignment_vu_snapshot_state_contract() -> dict[str, object]:
            return strategy_assignment_vu_snapshot_state_contract_payload()

        @app.post(
            "/api/strategies/assignment-vu-snapshot-state-validation",
            response_model=None,
        )
        async def strategies_assignment_vu_snapshot_state_validation(
            request: Request,
        ) -> JSONResponse:
        ...
        async def strategies_assignment_snapshot_context_validation(
            request: Request,
        ) -> JSONResponse:
        ...
        def run_control_strategy_period_chain_effect_probe_contract() -> dict[
            ...
            return strategy_execution_period_chain_effect_probe_contract_payload()

        @app.get(
            "/api/run-control/strategy-period-chain-five-period-effect-probe-contract"
        )
        def run_control_strategy_five_period_effect_probe_contract() -> dict[
            str, object
        ]:
        ...
        def run_control_strategy_candidate_effect_probe_contract() -> dict[
            ...
            return strategy_execution_candidate_effect_probe_contract_payload()

        @app.post(
            "/api/run-control/strategy-candidate-effect-probe",
            response_model=None,
        )
        async def run_control_strategy_candidate_effect_probe(
            request: Request,
        ) -> JSONResponse:
        ...
        async def run_control_strategy_period_chain_effect_probe(
            request: Request,
        ) -> JSONResponse:
            return await strategy_execution_period_chain_effect_probe_response(
                request
            )
        ...
        async def run_control_strategy_five_period_effect_probe(
            request: Request,
        ) -> JSONResponse:
        ...
        async def run_control_strategy_period_chain_effect_probe_start(
            request: Request,
        ) -> JSONResponse:
            return await (
                strategy_execution_period_chain_effect_probe_start_response(
                    request
                )
            )
        ...
        def run_control_strategy_period_chain_effect_probe_result(
            chain_id: str,
        ) -> JSONResponse:
            return strategy_execution_period_chain_effect_probe_result_response(
                chain_id
            )
        ...
        def run_control_strategy_period_chain_effect_probe_history(
            chain_id: str,
        ) -> JSONResponse:
            return strategy_execution_period_chain_effect_probe_history_response(
                chain_id
            )
    ...
    routes: list[Any] = [
        ...
        Route(
            "/api/model/life-sector-contract/v3",
            lambda request: JSONResponse(life_sector_v3_contract_payload()),
        ),
        Route(
        ...
        Route(
            "/api/accounting/solvency-risk-modules-contract",
            lambda request: JSONResponse(solvency_risk_modules_contract_payload()),
        ),
        Route(
        ...
        Route(
            "/api/strategies/execution-period-chain-five-period-build",
            strategy_execution_five_period_chain_build_response,
            methods=["POST"],
        ),
        Route(
        ...
        Route(
            "/api/strategies/execution-period-chains",
            lambda request: strategy_execution_period_chain_overview_response(),
        ),
        Route(
            "/api/strategies/execution-period-chains/{chain_id}",
        ...
        Route(
            "/api/strategies/assignment-vu-snapshot-state-contract",
            lambda request: JSONResponse(
                strategy_assignment_vu_snapshot_state_contract_payload()
            ),
        ),
        ...
        Route(
            "/api/run-control/strategy-period-chain-effect-probe",
            strategy_execution_period_chain_effect_probe_response,
            methods=["POST"],
        ),
        Route(
            "/api/run-control/strategy-period-chain-five-period-effect-probe",
        ...
        Route(
            "/api/run-control/strategy-period-chain-effect-probe-result/{chain_id}",
            lambda request: strategy_execution_period_chain_effect_probe_result_response(
                request.path_params["chain_id"]
            ),
        ),
        Route(
            "/api/run-control/strategy-period-chain-effect-probe-history/{chain_id}",
            lambda request: strategy_execution_period_chain_effect_probe_history_response(
                request.path_params["chain_id"]
            ),
        ),
        Route(
