...
CONTRACT_VERSION = "pr83-v1"
CONTRACT_BASE_SEED = 20260001
DEFAULT_CONTRACT_PATH = Path(
    "tests/fixtures/vdefmd6_vn_rule_group_1_run_contract.json"
)
DEFAULT_REFERENCE_PATHS = {
    ...
    "imsvnr03.dat": Path("tests/references/legacy_agrsich/IMSVNR03.DAT"),
}
_BLOCKER_CODES = (
...
class Vdefmd6VNRuleGroup1RunReport:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            "status": (
                "vn_rule_group_1_path_classified"
...
def build_vdefmd6_vn_rule_group_1_run_report(
    repo_root: Path | str,
    *,
    contract_path: Path | str | None = None,
    reference_paths: dict[str, Path | str] | None = None,
) -> Vdefmd6VNRuleGroup1RunReport:
    ...
    return Vdefmd6VNRuleGroup1RunReport(
        repo_root=str(root),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m ims.api.vdefmd6_vn_rule_group_1_run_report",
        description="Prueft die drei kontrollierten PR-83-VN-Regelaggregate.",
    )
    parser.add_argument("--repo-root", type=Path, default=Path.cwd())
    parser.add_argument("--contract", type=Path)
    args = parser.parse_args(argv)
    report = build_vdefmd6_vn_rule_group_1_run_report(
        args.repo_root,
...
def _build_target_summaries(
    ...
    if set(tables) != set(VDEFMD6_VN_RULE_GROUP_1_FILENAMES):
        _issue(
            issues,
        ...
        return ()
    if set(reference_paths) != set(VDEFMD6_VN_RULE_GROUP_1_FILENAMES):
        _issue(
            issues,
...
def _validate_contract(
    ...
    expected = {
        ...
        "model_id": "Vdefmd6",
        "scope": "controlled_vn_rule_group_1_periods_1_100",
