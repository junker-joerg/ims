...
CONTRACT_VERSION = "pr84-v1"
CONTRACT_BASE_SEED = 20260001
DEFAULT_CONTRACT_PATH = Path(
    "tests/fixtures/vdefmd6_vn_rule_group_2_run_contract.json"
)
DEFAULT_REFERENCE_PATHS = {
    "imsvnr04.dat": Path("tests/references/legacy_agrsich/IMSVNR04.DAT"),
    "imsvnr05.dat": Path("tests/references/legacy_agrsich/IMSVNR05.DAT"),
    "imsvnr06.dat": Path("tests/references/legacy_agrsich/IMSVNR06.DAT"),
}
_BLOCKER_CODES = (
...
class Vdefmd6VNRuleGroup2RunReport:
    repo_root: str
    contract_path: str
    target_summaries: tuple[dict[str, object], ...]
    source_anchor_count: int
    issues: tuple[Vdefmd6PreShockRunIssue, ...]
    mode: str = "vdefmd6_controlled_vn_rule_group_2_run"

    @property
    def rule_group_path_ready(self) -> bool:
        return not self.issues

    def to_dict(self) -> dict[str, object]:
        return {
            "status": (
                "vn_rule_group_2_path_classified"
                if self.rule_group_path_ready
                else "error"
            ),
            "mode": self.mode,
            ...
            "rule_target_count": len(self.target_summaries),
            ...
            "rule_group_generation_ready": self.rule_group_path_ready,
            "historical_comparison_classified": self.rule_group_path_ready,
            **_BOUNDARIES,
            "issues": [issue.to_dict() for issue in self.issues],
        }


def build_vdefmd6_vn_rule_group_2_run_report(
    repo_root: Path | str,
    *,
    contract_path: Path | str | None = None,
    reference_paths: dict[str, Path | str] | None = None,
) -> Vdefmd6VNRuleGroup2RunReport:
    ...
    return Vdefmd6VNRuleGroup2RunReport(
        repo_root=str(root),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m ims.api.vdefmd6_vn_rule_group_2_run_report",
        description="Prueft die drei kontrollierten PR-84-VN-Regelaggregate.",
    )
    parser.add_argument("--repo-root", type=Path, default=Path.cwd())
    parser.add_argument("--contract", type=Path)
    args = parser.parse_args(argv)
    report = build_vdefmd6_vn_rule_group_2_run_report(
        args.repo_root,
        contract_path=args.contract,
    )
    print(json.dumps(report.to_dict(), ensure_ascii=True, sort_keys=True))
    return 0 if report.rule_group_path_ready else 1
...
def _build_target_summaries(
    ...
    if set(tables) != set(VDEFMD6_VN_RULE_GROUP_2_FILENAMES):
        _issue(
            issues,
            "rule_target_set_mismatch",
            f"unexpected rule targets: {sorted(tables)}",
        )
        return ()
    if set(reference_paths) != set(VDEFMD6_VN_RULE_GROUP_2_FILENAMES):
        _issue(
            issues,
...
def _summarize_target(
    ...
    try:
        legacy_rows = {
            ...
            for row in parse_legacy_policyholder_dat(reference_path).rows
...
def _validate_contract(
    ...
    expected = {
        ...
        "model_id": "Vdefmd6",
        "scope": "controlled_vn_rule_group_2_periods_1_100",
