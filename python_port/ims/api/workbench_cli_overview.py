...
def build_workbench_cli_overview() -> WorkbenchCliOverviewResult:
    commands = (
        ...
        WorkbenchCliCommand(
            name="workbench_metadata_recovery inspect",
            command="python -m ims.api.workbench_metadata_recovery inspect --db .\\.ims_workbench\\metadata.sqlite --queue-id baseline-python-tests",
            purpose="Validierten lokalen Ergebnisstand rein lesend pruefen.",
            writes_enabled=False,
            requires_explicit_db=True,
        ),
        WorkbenchCliCommand(
            name="workbench_metadata_recovery backup",
            command="python -m ims.api.workbench_metadata_recovery backup --source-db .\\.ims_workbench\\metadata.sqlite --out .\\backup\\metadata.sqlite --queue-id baseline-python-tests",
            purpose="Explizites SQLite-Backup des validierten lokalen Ergebnisstands erzeugen.",
            writes_enabled=True,
            requires_explicit_db=True,
        ),
        WorkbenchCliCommand(
            name="workbench_metadata_recovery restore",
            command="python -m ims.api.workbench_metadata_recovery restore --backup-db .\\backup\\metadata.sqlite --out .\\restore\\metadata.sqlite --queue-id baseline-python-tests",
            purpose="Geprueftes SQLite-Backup in einen neuen expliziten Zielpfad wiederherstellen.",
            writes_enabled=True,
            requires_explicit_db=True,
        ),
        WorkbenchCliCommand(
            name="workbench_metadata_recovery verify",
            command="python -m ims.api.workbench_metadata_recovery verify --source-db .\\.ims_workbench\\metadata.sqlite --candidate-db .\\restore\\metadata.sqlite --queue-id baseline-python-tests",
            purpose="Quelle und Recovery-Kandidat rein lesend vergleichen.",
            writes_enabled=False,
            requires_explicit_db=True,
        ),
        WorkbenchCliCommand(
        ...
        WorkbenchCliCommand(
            name="run_control_queue_diagnostics",
            command="python -m ims.api.run_control_queue_diagnostics --db .\\.ims_workbench\\metadata.sqlite",
            purpose="Lokale Run-Control-Queue rein lesend diagnostizieren.",
            writes_enabled=False,
            requires_explicit_db=True,
        ),
        WorkbenchCliCommand(
