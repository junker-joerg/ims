...
- `python -m ims.api.workbench_artifact_manifest --root . --frontend-dist frontend/dist` beschreibt Ein- und Ausschlusspfade fuer ein spaeteres Artefakt.
...
Der Packaging-/Bereitstellungsblock bleibt grob bei ca. `4-10` reviewbaren PRs nach diesem Schritt:
...
5. Artefaktmanifest und Ausschluss lokaler Caches/Nutzerdaten: vorbereitet.
...
- Packaging und Bereitstellung: ca. `4-10` PRs.
- Integrations- und Review-Reserve: ca. `3-5` PRs.

Erwartet bleiben damit weiterhin grob ca. `26-43+` reviewbare PRs. Diese Zahl ist bewusst konservativ und kann durch Fachvalidierung oder Plattform-/Packaging-Fallen steigen.
...
- Artefaktmanifest fuer Ein- und Ausschlusspfade,
